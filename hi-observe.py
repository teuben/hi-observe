#! /usr/bin/env python
#
#  HI-observe:  Load a RA-DEC-VEL  FITS cube ,
...
#   parse our simple command line interface
na = len(sys.argv) - 1
...
# get a reference to the primary header and data. data better be 3-dim numpy array now
h = hdu[0].header
...
# pos[] is either a pixel or GLON/GLAT coordinate
xpos = pos[0]
ypos = pos[1]
...
if have_pixel:
    ...
    # now need to convert the xpos,ypos (in WCS) to pixel
...
#   finally we get the spectrum using a numpy slice operation, the Y coordinate in the plot
flux     = d[:,ypos,xpos]
